 ...
 * 
 * Diese Version speichert alle Daten im LocalStorage des Browsers.
 * Keine Server-Kommunikation, vollständig offline-fähig.
 * 
 * Um diese Version zu nutzen, setze in .env.local:
 * VITE_STORAGE_TYPE=local
 */
const useWorkoutStoreLocal = create(
  persist(
    (set, get) => ({
      // --- DATEN ---
    ...
    {
      name: 'muscle-app-storage-local', // Name im LocalStorage
      storage: createJSONStorage(() => localStorage), // Explizit LocalStorage nutzen
    }
  )
);

export default useWorkoutStoreLocal;
